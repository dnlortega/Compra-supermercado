...
async function main() {
  ...
  if (data.id) {
    ...
    if (existing) {
      ...
      // create new without id to avoid conflict
  ...
  console.log(`Imported shopping list '${createdList.name || createdList.id}' with ${createdCount} products. List id: ${createdList.id}`);
